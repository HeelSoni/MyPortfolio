...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-[#050508] text-[#ededed] py-32 px-6 flex flex-col items-center">
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-4xl h-[400px] bg-pink-900/20 rounded-[100%] blur-[120px] pointer-events-none" />
        ...
        <div className="glass p-8 md:p-12 rounded-3xl border border-white/10 shadow-2xl relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 relative z-10">
            <div className="md:col-span-2">
              <h2 className="text-3xl font-semibold mb-6 text-pink-300">Project Overview</h2>
            ...
            <div className="h-full flex flex-col gap-6">
              <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {['Python', 'Scikit-Learn', 'Pandas', 'SQL', 'Power BI'].map(tech => (
                    <span key={tech} className="px-3 py-1 bg-pink-500/20 text-pink-200 text-sm rounded-full border border-pink-500/30">
                      {tech}
                    </span>
                  ))}
                </div>
              ...
              <div className="bg-white/5 border border-white/10 rounded-2xl p-6 flex-grow flex items-center justify-center">
                 <div className="text-center">
                   <div className="w-16 h-16 rounded-full border-4 border-pink-500 border-t-transparent animate-spin mx-auto mb-4 opacity-50" />
          ...
          <div className="p-6 mt-8 bg-pink-900/20 rounded-xl border border-pink-500/20 text-center relative z-10">
            <h4 className="text-pink-300 font-medium mb-2">Want to see the actual code repository?</h4>
            ...
            <a href="https://github.com/search?q=Data+Analysis+Portfolio&type=repositories" target="_blank" rel="noopener noreferrer" className="inline-block px-6 py-2 bg-pink-600 hover:bg-pink-500 text-white rounded-lg transition-colors text-sm font-medium">
